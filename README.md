...
for more info.
BUILD FOR VITA: run install_all in the vitasdk and then make sure you have python2 and scons then run this build coammand: scons platform=vita target=debug -j4 module_squish_enabled=false builtin_openssl=no module_openssl_enabled=false disable_3d=yes module_webp_enabled=false editor=no
